...
- (void)addNewBackItem
{
    ...
    firstButton.frame = CGRectMake(0, 0, 60, 60);
//    firstButton.contentEdgeInsets =UIEdgeInsetsMake(0, -20,0, 0);
//    firstButton.imageEdgeInsets =UIEdgeInsetsMake(0, -15,0, 0);
    [firstButton setImage:[[UIImage iconWithInfo:TBCityIconInfoMake(@"\U0000e916",25,C000000)] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal] forState:normal];
    ...
    if ([self.dataSource respondsToSelector:@selector(numberViewControllersInViewPager:)]) {
        ...
        for (int i = 0; i<totalVC; i++) {
            ...
            if ([self.dataSource respondsToSelector:@selector(viewPager:titleWithIndexViewControllers:)]) {
                ...
                button.frame = CGRectMake(oldRect.origin.x+oldRect.size.width, 0, _btnTitleWidth?_btnTitleWidth:[self textString:buttonTitle withFontHeight:20], [self.dataSource respondsToSelector:@selector(heightForTitleViewPager:)]?[self.dataSource heightForTitleViewPager:self]:0);
//                [button setTitleEdgeInsets:UIEdgeInsetsMake(0, 0, 0, -FITiPhone6(20))];
...
{
    if (self.selected) {
        CGFloat lineWidth = FITiPhone6(2);
        CGColorRef color = self.titleLabel.textColor.CGColor;
        CGContextRef ctx = UIGraphicsGetCurrentContext();
        CGContextSetStrokeColorWithColor(ctx, color);
        CGContextSetLineWidth(ctx, lineWidth);
        CGContextMoveToPoint(ctx, FITiPhone6(30), self.frame.size.height-lineWidth);
        CGContextAddLineToPoint(ctx, self.frame.size.width-FITiPhone6(27.5), self.frame.size.height-lineWidth);
