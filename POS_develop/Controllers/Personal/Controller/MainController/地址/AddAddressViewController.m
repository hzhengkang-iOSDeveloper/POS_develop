...
- (void)saveClick {
    ...
    [[HPDConnect connect] PostNetRequestMethod:@"api/trans/address/save" params:@{@"userid":IF_NULL_TO_STRING(manager.userInfo.userId), @"defaultFlag":self.defaultAddressBtn.selected?@0:@1, @"receiverName":self.nameTF.text, @"receiverMp":self.telephoneTF.text, @"province" : province, @"city" : city, @"county": county, @"receiverAddr":self.detailAddress.text} cookie:nil result:^(bool success, id result) {
        if (success) {
