...
- (void)setElementCount:(NSInteger)elementCount {
    ...
    for (int i = 0; i < self.elementCount; i++)
    {
        ...
        //        pwdTextField.layer.borderWidth  = FITiPhone6(2);
        //        pwdTextField.textColor = DeepTextColor;
        //        pwdTextField.layer.borderColor = TextColor_Gray.CGColor;
...
- (void)layoutSubviews {
    ...
    [self.dataSource removeAllObjects];
    for (NSUInteger i = 0; i < newDataSource.count; i++) {
        UITextField *pwdTextField = [newDataSource objectAtIndex:i];
        x = i * (w + self.elementMargin-FITiPhone6(0.5));
        pwdTextField.frame = CGRectMake(x, y, w, h);
        UIBezierPath *maskPath;
        if (i == 0 || i == self.elementCount -1) {
            
            if (i == 0) {
                maskPath = [UIBezierPath bezierPathWithRoundedRect:pwdTextField.bounds byRoundingCorners:UIRectCornerTopLeft | UIRectCornerBottomLeft cornerRadii:CGSizeMake(FITiPhone6(12), FITiPhone6(12))];
            } else {
                maskPath = [UIBezierPath bezierPathWithRoundedRect:pwdTextField.bounds byRoundingCorners:UIRectCornerTopRight | UIRectCornerBottomRight cornerRadii:CGSizeMake(FITiPhone6(12), FITiPhone6(12))];
            }
            
            
        } else {
            maskPath = [UIBezierPath bezierPathWithRoundedRect:pwdTextField.bounds byRoundingCorners:UIRectCornerTopLeft | UIRectCornerBottomLeft cornerRadii:CGSizeMake(0, 0)];
        }
        //边框
        ...
        maskLayer.frame = pwdTextField.bounds;
