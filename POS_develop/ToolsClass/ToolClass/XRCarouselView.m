...
//  XRCarouselView.m
//
//  Created by 邝子涵 on 16/7/03.
//  Copyright © 2016年 邝子涵. All rights reserved.
//
...
- (void)setImageArray:(NSArray *)imageArray{
    ...
    for (int i = 0; i < imageArray.count; i++) {
        if ([imageArray[i] isKindOfClass:[UIImage class]]) {
            ...
            [_images addObject:ImageNamed(@"背景")];
...
- (void)downloadImages:(int)index {
    ...
    if (imageName.length > 20) {
        imageName = [imageName substringFromIndex:20];
    }
    ...
    if (_autoCache) {
        ...
        if (data) {
            if ([_images isKindOfClass:[NSMutableArray class]] && _images.count > index) {
    ...
    NSBlockOperation *download = [NSBlockOperation blockOperationWithBlock:^{
        ...
        //取到的data有可能不是图片
        if (image) {
            if ([_images isKindOfClass:[NSArray class]] && _images.count > index) {
