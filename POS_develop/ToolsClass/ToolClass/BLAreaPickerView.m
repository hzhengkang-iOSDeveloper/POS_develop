...
- (void)sureButtonClicked:(UIButton *)sender{
    ...
    if (areaArr.count != 0) {
        _region_id = [NSString stringWithFormat:@"CN%@",[areaArr[_areaSelectedRow] objectForKey:@"id"]];
    } else {
        _region_id = [NSString stringWithFormat:@"CN%@",[cityArr[_citySelectedRow] objectForKey:@"id"]];
    }
    
    
    if (self.pickViewDelegate &&
        [self.pickViewDelegate respondsToSelector:@selector(bl_selectedAreaResultWithProvince:city:area:region_id:)]) {
